...
@section('title')
    الأسئلة الشائعة
@endsection
@section('page_name')  الأسئلة الشائعة @endsection
@section('content')
    <div class="row">
        ...
        <div class="modal fade bd-example-modal-lg" id="editOrCreate" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="example-Modal3">الأسئلة الشائعة</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('faqs.index')}}', columns);
        // Delete Using Ajax

        deleteAny('{{route('faqs.destroy', ':id')}}');

        // Add Using Ajax
        showAddModal('{{route('faqs.create')}}');
        addScript();



        // Edit Using Ajax
        showEditModal('{{route('faqs.edit',':id')}}');
        editScript();
    </script>
@endsection
